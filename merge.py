...
async def main():
    ...
    for i, scene in enumerate(scenes):
        ...
        try:
            ...
            if motion_type == "zoom_in":
                img_clip = img_clip.resize(height=1080).resize(lambda t: 1 + 0.015 * t).set_position('center')
            elif motion_type == "zoom_out":
                img_clip = img_clip.resize(height=1150).resize(lambda t: 1 - 0.015 * t).set_position('center')
            elif motion_type == "pan_left":
                img_clip = img_clip.resize(height=1150).set_position(lambda t: (int(-15 * t), 'center'))
            elif motion_type == "pan_right":
                img_clip = img_clip.resize(height=1150).set_position(lambda t: (int(-75 + 15 * t), 'center'))
    ...
    if os.path.exists(bg_music_path):
        ...
        print("⚠️ bg.mp3 file nahi mili! Video bina music ke banegi.")
